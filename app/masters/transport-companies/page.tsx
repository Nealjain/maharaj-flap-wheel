...
export default function TransportCompaniesPage() {
  ...
  const handleDeleteCompany = async (companyId: string, companyName: string) => {
    ...
    try {
      const { error } = await deleteTransportCompany(companyId)

      if (error) {
        console.error('Error deleting transport company:', error)
        // Check if it's a foreign key constraint error
        if (error.message?.includes('foreign key') || error.code === '23503') {
          addToast('Cannot delete: Company is used in orders.', 'error')
        } else {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Manage logistics and shipping partners
            </p>
          </div>
          <div className="mt-4 sm:mt-0">
            <button
              onClick={() => router.push('/masters/transport-companies/create')}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2">
              <CSVExport
                data={exportData}
                headers={['Name', 'Address', 'Phone', 'Created At', 'Updated At']}
                filename="transport_companies"
                className="px-4 py-2"
              />
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          {filteredCompanies.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Get started by adding your first transport company.
              </p>
              <div className="mt-6">
                <button
                  onClick={() => router.push('/masters/transport-companies/create')}
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => router.push(`/masters/transport-companies/${company.id}/edit`)}
