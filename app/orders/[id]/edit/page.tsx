...
export default function EditOrderPage({ params }: EditOrderPageProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // Note: This is a simplified update - in production you'd want to handle
      // order_items updates more carefully (delete old ones, insert new ones)
      const orderData = {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 shadow-lg rounded-xl border border-gray-200 dark:border-gray-700 p-6">
              ...
              {orderItems.map((orderItem) => (
                <div key={orderItem.item_id} className="flex items-center space-x-4 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">SKU: {orderItem.sku}</p>
                  </div>
                  
                  <div className="flex items-center space-x-2">
                    <label className="text-sm text-gray-700 dark:text-gray-300">Qty:</label>
                    <span className="text-sm font-medium text-gray-900 dark:text-white">{orderItem.quantity}</span>
                  </div>
                  
                  <div className="flex items-center space-x-2">
                    <label className="text-sm text-gray-700 dark:text-gray-300">Price:</label>
                    <span className="text-sm font-medium text-gray-900 dark:text-white">₹{orderItem.price.toFixed(2)}</span>
                  </div>
            ...
            <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">
              Note: Order items cannot be modified after creation. To change items, please create a new order.
            </p>
          </div>
